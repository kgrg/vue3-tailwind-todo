  ...
  <BaseListItem>
    <template #leading>
      <button
        ...
        :class="[
          isCompleted ? 'bg-blue-500 border-blue-500' : 'border-gray-300 hover:border-blue-500',
        ]"
        @click="$emit('toggle-complete')"
      >
        <CheckIcon v-if="isCompleted" class="w-full h-full text-white p-0.5" />
      </button>
    </template>

    <template #title>
      <span class="text-sm font-medium" :class="{ 'line-through text-gray-400': isCompleted }">
        {{ title }}
      </span>
    ...
    <template #actions>
      ...
      >
        <StarIcon class="w-5 h-5" :class="{ 'fill-yellow-500 text-yellow-500': isImportant }" />
      </button>
      <button
        type="button"
...
<script setup>
import { CheckIcon, StarIcon, CalendarIcon, PencilIcon, TrashIcon } from '@/components/icons'
